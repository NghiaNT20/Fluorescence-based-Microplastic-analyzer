...
## Thông điệp dùng khi trình bày

Benchmark hiện tại đã cải thiện đáng kể so với cách so sánh chỉ dựa trên số lượng
particle. Công thức IoU, TP, FP, FN, precision, recall và F1 cơ bản là đúng trong
các trường hợp đơn giản. Tuy nhiên, toàn bộ quy trình **chưa đủ điều kiện để đưa ra
kết luận cuối cùng về chất lượng model**.

Nguyên nhân không nằm ở một công thức duy nhất. Kết quả còn bị ảnh hưởng bởi lỗi
tạo report, cách xác nhận ground truth, thuật toán ghép box, ý nghĩa class-agnostic
của metric, coverage bị ẩn, dữ liệu synthetic, nguy cơ lẫn train/test và protocol
đo thời gian chưa công bằng.

> [!WARNING]
> Không dùng kết quả benchmark hiện tại làm số liệu công bố, tiêu chí nghiệm thu,
> quyết định chọn model hoặc khẳng định model tốt hơn Quick và Deep. Trước tiên
> cần xử lý toàn bộ vấn đề P0 và P1, sau đó chạy lại trên một test set cố định.
...
## Mẫu kết luận sau khi benchmark được sửa

Có thể dùng cấu trúc sau khi trình bày kết quả:

```text
Dataset test gồm [N] ảnh và [M] particle đã xác thực.
Evaluation coverage đạt [X]%, với [K] ảnh bị loại vì [lý do].
Model được đánh giá tại confidence [C] và IoU [T].
Localization F1 đạt [LF1], class-aware F1 đạt [CF1], AP50 đạt [AP50].
End-to-end latency median là [P50] ms và p90 là [P90] ms trên [device].
Không phát hiện trùng lặp giữa train, validation và test theo SHA-256.
Kết luận áp dụng cho [synthetic/real/mixed] dataset version [ID].
```

Không nên trình bày một giá trị F1 riêng lẻ mà thiếu dataset version, coverage, class
semantics, confidence threshold, IoU threshold và thiết bị thực thi.

## Evidence kiểm chứng tại thời điểm biên soạn

Ngày kiểm tra: 2026-08-22.

* `compileall` cho `main.py`, `config` và `src` đã đạt
* `main.py --system-info` đã chạy thành công
* 14 unit test trong `tests/unit/` đã đạt bằng `unittest`
* Counterexample greedy matching đã trả TP=1 dù tồn tại phương án TP=2
* `pytest` không khả dụng trong môi trường kiểm tra
* PyTorch và Ultralytics không khả dụng hoặc không tương thích trong môi trường kiểm tra
* ML benchmark runtime thực tế chưa được xác minh
* Các vấn đề trong tài liệu là hiện trạng chưa được sửa

> [!NOTE]
> Tài liệu được biên soạn với hỗ trợ AI dựa trên source code tại thời điểm kiểm tra.
> Cần human review trước khi dùng cho công bố khoa học, nghiệm thu hoặc quyết định
> lựa chọn model.
